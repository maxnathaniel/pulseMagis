  ...
  return(
    <div style={{display:'flex',flexWrap:'wrap',gap:'10px 16px',justifyContent:'center',padding:'10px 0'}}>
      {entries.map(([word,count],i)=>(
        <span key={word} style={{fontFamily:FONT_DISPLAY,fontWeight:500,fontSize:14+(count/max)*28,color:PALETTE_BARS[i%PALETTE_BARS.length],animation:`fadeUp .4s ease ${i*30}ms both`,lineHeight:1}}>{word}</span>
      ))}
    </div>
  )
}
