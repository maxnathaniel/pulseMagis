    ...
    <div style={{display:'flex',flexDirection:'column',gap:8}}>
      {options.map((opt,i)=>(
        <div key={i} style={{display:'flex',alignItems:'center',gap:8}}>
          ...
          <span style={{flex:1,fontSize:19,fontWeight:500,color:C.txt1}}>{opt}</span>
          <span style={{fontSize:12,fontWeight:500,color:C.txt3}}>{counts[i]} · {total?Math.round(counts[i]/total*100):0}%</span>
        </div>
      ))}
    </div>
  )
