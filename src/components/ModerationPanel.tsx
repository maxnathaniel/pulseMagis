...
interface ModerationPanelProps {
  ...
  // True on the live/projected presenter screen, where the room can see
  // whatever's rendered here — moderation controls and the pending queue
  // must stay host-only, so this collapses the panel down to just the
  // already-approved list (and hides it silently, with no placeholder
  // text, when that list is empty).
  audienceView?: boolean
}

export function ModerationPanel({session,qnaList,onModerate,onToggleModeration,audienceView}: ModerationPanelProps){
  ...
  if (audienceView&&visible.length===0) return null
  return(
    <div style={{flex:1,overflowY:'auto',padding:'18px 2px 10px',display:'flex',flexDirection:'column',gap:22}}>
      ...
      {session.qnaModeration&&!audienceView&&(
        <div>
      ...
      <div>
        {!audienceView&&<SectionLabel>Questions {visible.length?`(${visible.length})`:''}</SectionLabel>}
        {visible.length===0?(audienceView?null:<EmptyState text="Approved questions will appear here, ranked by votes."/>):(
          <div style={{display:'flex',flexDirection:'column',gap:8,marginTop:10}}>
            {visible.map(q=>(
