...
import { C, FONT_BODY, useFonts, EMPTY_RICH_DOC } from './theme.js'
import { uid, genCode, hashPin, mapSlide, mapSlideForBuilder, mapQuestion } from './lib/helpers.js'
import { Home } from './screens/Home/Home.jsx'
import { Login } from './screens/Login/Login.jsx'
import { Builder } from './screens/Builder/Builder.jsx'
import { Presenter } from './screens/Presenter/Presenter.jsx'
import { Join } from './screens/Join/Join.jsx'
import { Vote } from './screens/Vote/Vote.jsx'
...
export default function App() {
  ...
  const createSlide = (type) =>
    type==='choice'
      ? { id:uid(), type, question:'', options:['','','',''], optionImages:[null,null,null,null],
          layout:'right', contentImage:null, responseMode:'instant' }
      : type==='plain'
      ? { id:uid(), type, question:'', layout:'right', contentImage:null, responseMode:'instant',
  ...
  const changeSlideType = (id, newType) =>
    setDraft(d => ({ ...d, slides: d.slides.map(s => {
      ...
      if (newType==='choice') return { id:s.id, type:newType, ...shared,
        ...
        optionImages: s.optionImages?.length ? s.optionImages : [null,null,null,null] }
      if (newType==='plain') return { id:s.id, type:newType, ...shared,
        content: s.content || EMPTY_RICH_DOC, verticalAlign: s.verticalAlign || 'middle' }
      return { id:s.id, type:newType, ...shared }
    })}))
  const applyResponseModeToAll = (mode) =>
  ...
  const addOption = (sid) =>
    setDraft(d => ({ ...d, slides: d.slides.map(s => s.id===sid && s.options.length<6
      ? {...s, options:[...s.options,''], optionImages:[...(s.optionImages||[]),null]} : s) }))
  const removeOption = (sid,oi) =>
    setDraft(d => ({ ...d, slides: d.slides.map(s => s.id===sid && s.options.length>2
      ? {...s, options:s.options.filter((_,i)=>i!==oi), optionImages:(s.optionImages||[]).filter((_,i)=>i!==oi)} : s) }))
  const updateOption = (sid,oi,val) =>
    setDraft(d => ({ ...d, slides: d.slides.map(s => s.id===sid
  ...
  const startPresenting = async () => {
    ...
    await supabase.from('slides').insert(draft.slides.map((s,idx) => ({
      ...
      option_images:s.optionImages||null, position:idx,
      layout:s.layout||'right', content_image:s.contentImage||null, response_mode:s.responseMode||'instant',
      content:s.type==='plain'?(s.content||null):null, vertical_align:s.verticalAlign||'middle',
    }))).then(({error}) => { if (error) console.error(error) })
    setSession({ code, title,
      slides:draft.slides.map(s=>({...s,options:s.type==='choice'?s.options.filter(o=>o.trim()):s.options})),
  ...
  const resumePresenting = async (code) => {
    const {data:sd,error}=await supabase.from('sessions').select('*').eq('code',code).single()
    if (error||!sd) { console.error(error); return }
    const {data:slideRows}=await supabase.from('slides').select('*').eq('session_code',code).order('position')
  ...
  const renamePulse = async (code, title) => {
  ...
  // ── realtime: Q&A ─────────────────────────────────────────────────────────
  const fetchQna = useCallback(async (code) => {
  ...
  const goToSlideInFlight = useRef(null)
  const goToSlide=async(newIdx)=>{
    if (!session||newIdx<0||newIdx>=session.slides.length) return
    setSlideIndex(newIdx); setSession(p=>({...p,currentSlideIndex:newIdx}))
  ...
  const submitJoin=async(codeOverride)=>{
  ...
  const moderateQuestion=async(qId,action)=>{
    if (!session) return
  ...
  const toggleModeration=async()=>{
    ...
    setSession(p=>({...p,qnaModeration:newMod}))
  ...
  return (
    <div style={{height:'100vh',width:'100%',overflow:'hidden',background:C.pageBg,color:C.txt1,fontFamily:FONT_BODY,display:'flex',flexDirection:'column'}}>
      ...
      {loadingAuth
        ...
        : (['home','build','present'].includes(screen) && !user)
