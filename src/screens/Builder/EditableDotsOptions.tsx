        ...
        return(
          <div key={i}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                ...
                <input value={opt} onChange={e=>onUpdateOption?.(i,e.target.value)} placeholder={`Option ${i+1}`}
                  style={{flex:1,minWidth:0,background:'transparent',border:'none',outline:'none',color:C.txt1,
                    fontFamily:FONT_DISPLAY,fontWeight:500,fontSize:20,borderBottom:`1.5px solid ${C.border}`,padding:'2px 0'}}/>
            ...
            <div style={{display:'flex',alignItems:'center',gap:12}}>
              ...
              {!readOnly&&<span style={{fontSize:12,fontWeight:500,color:C.txt3}}>{c} · {pct}%</span>}
            </div>
          </div>
        )
      })}
