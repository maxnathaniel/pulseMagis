    ...
    <div style={{flex:1,minHeight:0,display:'flex',flexDirection:'column',justifyContent:'center',maxWidth:480,margin:'0 auto',width:'100%'}}>
      ...
      {slide.type==='qa'?(
        ...
        <div style={{textAlign:'center',padding:'30px 10px'}}>
          ...
          <div style={{fontFamily:FONT_DISPLAY,fontWeight:500,fontSize:19,color:C.txt1}}>Response submitted!</div>
          <div style={{color:C.txt3,fontSize:14,marginTop:6,fontWeight:400}}>Check the presentation screen for live results.</div>
        </div>
      ):(
        <>
          {slide.type==='choice'&&(()=>{
                    ...
                    <button key={i} onClick={()=>setChoiceInput(i)}
                      ...
                      <div style={{width:58,height:58,borderRadius:'50%',margin:'0 auto 10px',border:`2.5px solid ${selected?color:C.border}`,overflow:'hidden',background:img?'transparent':`${color}18`,display:'flex',alignItems:'center',justifyContent:'center'}}>
                        {img?<img src={img} alt={opt} style={{width:'100%',height:'100%',objectFit:'cover'}}/>
                            :<span style={{fontFamily:FONT_DISPLAY,fontSize:22,fontWeight:500,color}}>{opt.charAt(0).toUpperCase()}</span>}
                      </div>
                      <div style={{fontSize:13,fontWeight:500,fontFamily:FONT_DISPLAY,color:selected?color:C.txt1}}>{opt}</div>
                    </button>
                  )
                })}
              </div>
                  ...
                  return(
                    <button key={i} onClick={()=>setChoiceInput(i)}
                      style={{textAlign:'left',padding:'14px 16px',borderRadius:9999,
                        ...
                        color:selected?color:C.txt1,fontSize:15,fontWeight:500,cursor:'pointer',
          ...
          {slide.type==='wordcloud'&&(
            <input value={textInput} onChange={e=>setTextInput(e.target.value.slice(0,30))} placeholder="One word or short phrase"
              style={{width:'100%',padding:'14px 16px',borderRadius:4,border:`2px solid ${C.border}`,background:C.surface,color:C.txt1,fontSize:15,fontWeight:500,outline:'none',boxShadow:C.shadow}}/>
          )}
          {slide.type==='open'&&(
            <textarea value={textInput} onChange={e=>setTextInput(e.target.value.slice(0,140))} placeholder="Type your response…" rows={4}
              style={{width:'100%',padding:'14px 16px',borderRadius:4,border:`2px solid ${C.border}`,background:C.surface,color:C.txt1,fontSize:15,fontWeight:500,outline:'none',resize:'none',fontFamily:FONT_BODY,boxShadow:C.shadow}}/>
          )}
          {(() => {
            ...
            return(
              <button onClick={onSubmit} disabled={disabled}
                style={{marginTop:20,padding:'14px',borderRadius:9999,border:'none',
                  ...
                  fontFamily:FONT_DISPLAY,fontWeight:500,fontSize:16,cursor:disabled?'not-allowed':'pointer',
