    ...
    return(
      <div style={{flex:1,display:'flex',flexDirection:'column',padding:'18px',minHeight:0}}>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',textAlign:'center',padding:'0 24px'}}>
          ...
          <div style={{color:C.txt3,fontSize:14,marginTop:6,fontWeight:400}}>The presenter has ended this presentation. Thanks for moderating!</div>
        </div>
      </div>
    )
  }
    ...
    return(
      <div style={{flex:1,display:'flex',flexDirection:'column',padding:'18px'}}>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}>
          ...
          <h2 style={{fontFamily:FONT_DISPLAY,fontSize:28,fontWeight:500,margin:'0 0 6px',color:C.txt1}}>Enter the moderator PIN</h2>
          <p style={{color:C.txt3,fontSize:14,marginBottom:24,fontWeight:400}}>Ask the presenter if you don't have it</p>
          <input value={pinInput} onChange={e=>setPinInput(e.target.value.slice(0,20))} onKeyDown={e=>e.key==='Enter'&&submitPin()}
            ...
            style={{width:290,textAlign:'center',fontFamily:FONT_DISPLAY,fontSize:34,fontWeight:500,letterSpacing:7,background:C.surface,border:`2px solid ${C.border}`,borderRadius:5,color:C.txt1,padding:'14px 10px',outline:'none',boxShadow:C.shadow}}/>
          {error&&<div style={{color:C.red,fontSize:13,marginTop:14,maxWidth:260,textAlign:'center',fontWeight:500}}>{error}</div>}
          <button onClick={submitPin} disabled={checking||!pinInput.trim()}
            style={{marginTop:24,padding:'13px 40px',borderRadius:9999,border:'none',background:C.teal,color:'#fff',fontFamily:FONT_DISPLAY,fontWeight:500,fontSize:16,cursor:checking?'wait':'pointer',boxShadow:`0 4px 16px ${C.tealBg}`}}>
  ...
  return(
    <div style={{flex:1,display:'flex',flexDirection:'column',padding:'18px',minHeight:0}}>
      ...
      <div style={{flex:1,minHeight:0,display:'flex',flexDirection:'column',maxWidth:560,width:'100%',margin:'0 auto'}}>
        <h2 style={{fontFamily:FONT_DISPLAY,fontSize:'clamp(20px,3vw,26px)',fontWeight:500,margin:'6px 0 12px',color:C.txt1}}>{session.title}</h2>
